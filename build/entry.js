...
const promiseScraper = async (input, type, options = {}, maxCursor) => {
    ...
    const constructor = Object.assign(Object.assign(Object.assign({}, getInitOptions()), options), { type, input, maxCursor });
...
exports.hashtag = async (input, options, maxCursor) => promiseScraper(input, 'hashtag', options, maxCursor);
exports.user = async (input, options, maxCursor) => promiseScraper(input, 'user', options, maxCursor);
exports.trend = async (input, options, maxCursor) => promiseScraper(input, 'trend', options, maxCursor);
exports.music = async (input, options, maxCursor) => promiseScraper(input, 'music', options, maxCursor);
    ...
    return new Promise(resolve => {
        ...
        async_1.forEachLimit(batch, options.asyncBulk || 5, async (item) => {
            switch (item.type) {
                case 'user':
                    try {
                        const output = await exports.user(item.input, Object.assign({ bulk: true }, options), item.maxCursor);
                ...
                case 'hashtag':
                    try {
                        const output = await exports.hashtag(item.input, Object.assign({ bulk: true }, options), item.maxCursor);
                        result.push({ type: item.type, input: item.input, completed: true, scraped: output.collector.length });
                        console.log(`Scraping completed: ${item.type} ${item.input}`);
                    }
                    catch (error) {
                ...
                case 'music':
                    try {
                        const output = await exports.music(item.input, Object.assign({ bulk: true }, options), item.maxCursor);
                        result.push({ type: item.type, input: item.input, completed: true, scraped: output.collector.length });
                        console.log(`Scraping completed: ${item.type} ${item.input}`);
                    }
                    catch (error) {
